...
function Pcart(props){
    ...
    return(
        ...
        <div className='bg-white p-5 rounded-lg  shadow-sm'>
          ...
          <Link  to={slug}>
            <br></br>
      <img   src={imgurl} alt='' className='h-60 w-60 object-contain rounded-xl object-top hover:scale-105' ></img> 
      </Link>
       ...
       className={cbot===i?dark:light}>{i}</div>   </>))}
         ...
         <div className='flex justify-center items-center'>
          <button className='text-sm text-white bg-slate-950 rounded-xl
            ...
            ' type='button' 
            onClick={cbot===0?handleAdd:cbot>0?alert('item added'):''} >     <FaCartPlus/>ADD TO CART</button>
         </div>
         </div>
    )
}
