...
function Citem(props){
   const {_id,slug,imgurl,price,count,quantity}=props.data
   const [q,setQ]=useState(props.data.quantity)
   const dispatch=useDispatch()
    const handleAdd=()=>{
          dispatch(changeQ({id:_id,quantity:q,price:price}))
          setQ(quantity+1) }
   
       const handleRem=()=>{
         q===0?dispatch(remItem(_id)):changeQ(_id,q)
    ...
    <div  className="grid grid-cols-1 gap-3" >
      <div className="inline-flex border-2 gap-7 bg-gray-800 w-full">
        ...
        <span  className=' rounded-sm  text-sm h-10 w-10 justify-center
         text-center hover:scale-105'>{slug}</span>
        <h4 className="text-sm text-white"><FaRupeeSign/>{price}</h4>
